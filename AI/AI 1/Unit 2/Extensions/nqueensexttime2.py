...
def chooseNext(state,N,row):
    """
    Precondition: row is None
    Iterates through each row. determines the row with minconflict
    """
    minconf = N+100
    col = []
    for i in range(N):
        conf = count_conflict(state,N,row,i)
        if conf == minconf:
            col.append(i)
        elif conf < minconf:
            col = [i]
...
def count_conflict(state,N,row,col):
    """
...
def total_conflict(state,N):
    count = 0
    for i in range(N):
        count += count_conflict(state,N,i,state[i])
    return count//2
def goal_test(state, N):
    if len(state) > N:
        return False
...
def inc_repair(state,N,conflict):
    ...
    while conflict > 0:
        ...
        biggest = 0
        for i in range(N):
            count = count_conflict(state,N,row)
            if count == biggest:
                max_rows.append(i)
            elif count > biggest:
                biggest = count
