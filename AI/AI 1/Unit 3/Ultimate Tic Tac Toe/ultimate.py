...
def scoreBoard(board,player):
    ...
    if endGame is not None:
        ...
        big_Score = score_subboard(big_board,player)*9
        smallScore = 0
        for b in board:
            smallScore += score_subboard(b,player)
        return big_Score + smallScore
...
def max_step(board,player,depth,alpha,beta,max_depth,last_played):
    ...
    for move in nextPossMoves:
        ...
        next_poss = min_step(board,other,depth+1,alpha,beta,max_depth,move)
        if next_poss > alpha:
            alpha = next_poss
...
def min_step(board,other,depth,alpha,beta,max_depth,last_played):
    ...
    for move in nextPossMoves:
        nb = board.copy()
        make_move(nb,move,player)
        next_poss = max_step(board,other,depth+1,alpha,beta,max_depth,move)
        if next_poss < beta:
            beta = next_poss
        result.append(next_poss)
    return min(result)
    
def bestPlayer(board,last_played,player):
    other = "O"
    if player == "O":
        other = "X"
    alpha = -float("inf")
    ...
    for move in nextPossMoves:
        nb = board.copy()
        make_move(nb,move,player)
        result = min_step(board,other,1,alpha,beta,4,move)
        if result > alpha:
            alpha = result
        if result > max_score:
            max_score = result
...
last_played = -1
turn = 0
# print(next_move(board,user,last_played))
# make_move(board,2,user)
# printBoard(board)
# print(next_move(board,other,2))
while True:
    if turn %2 == 0:
        print("Current board: ")
        printBoard(board)
        nextPoss = next_move(board,user,last_played)
        print("Next",nextPoss)
        if len(nextPoss) == 0:
            print("Game Over!")
            break
        ...
        last_played = choice
        board = make_move(board,choice,user)
        turn += 1
    else:
        print("Current board: ")
        printBoard(board)
        nextPoss = next_move(board,other,last_played)
        print(nextPoss)
        if nextPoss is None:
            print("Game Over!")
            break
        choice = bestPlayer(board,last_played,other)
        last_played = choice
        board = make_move(board,choice,other)
        turn += 1
     
# print(next_move(test_board,"O",-1))