...
def scoreGame(board,player):
    ...
    mobility = (len(ptokens) - len(otokens))/(len(ptokens)+len(otokens))*200 #*(1-(pcount+ocount)/64) # mobility
    score += mobility
    # print("mobility",mobility)
    tokensscore = (pcount-ocount)/(pcount+ocount)*200*(pcount+ocount)/64# *(1+(pcount+ocount)/64)
    score += tokensscore
    corners_dict = {
    0: {1, 8, 9},
    7: {6, 14, 15},
    ...
    if pedges + oedges!=0:
        score += (pedges-oedges)/(pedges+oedges)*100
    for c in corners_dict:
        ...
        for sq in corners_dict[c]:
            if board[sq] == player:# and board[c] !=player: 
                pcorneradj += 1
            elif board[sq] == other:# and board[c] !=player:
                ocorneradj += 1
    # print("pcorners,ocorners,pcorneradj,ocorneradj",pcorners,ocorners,pcorneradj,ocorneradj)
    if pcorners + ocorners!=0:
        conrerscore = 200*(pcorners-ocorners)/(pcorners+ocorners)
        # print("Coners",conrerscore)
        score += conrerscore
    if pcorneradj + ocorneradj !=0:
        corneradjscore = 75*(pcorneradj-ocorneradj)/(pcorneradj+ocorneradj)
...
# for testing with mr. eckel's script
board = sys.argv[1]
player = sys.argv[2]
# board = "oooooo...oooo.x..oooox....xxxx....xxxx......x.......x.......x..."
# player = "o"
# # # # # board = "...........................ox......xo..........................."
# # # # # player = "o"
depth = 1
