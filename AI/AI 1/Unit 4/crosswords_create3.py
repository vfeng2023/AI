import sys; args = ["twentyk.txt","3x3","0"]# sys.argv[1:]
...
def backtrackFill(board,verttoIndex,horzToIndex,indexToVert,indexToHorz,horzToWords,vertToWords,all_possible_letters): # and other args
    ...
    for letter in possible_letters:
        ...
        new_all_poss = all_possible_letters.copy()
        new_all_poss[next] = set([letter])
        # flag keeps track if successfully updated
        # checked = update(newboard,next,newhor2words,newvert2words,horzToWords,vertToWords,indexToHorz,indexToVert,horzToIndex,verttoIndex,newused)
        checked,newHorz,newVert = forward_look(newboard,[next],indexToVert,indexToHorz,horzToIndex,verttoIndex,new_all_poss,horzToWords,vertToWords)
        # place letter and structures
        if checked is not None:
            result = backtrackFill(checked,verttoIndex,horzToIndex,indexToVert,indexToHorz,newHorz,newVert,new_all_poss)
            if result is not None:
                return result

def update(board,index,newhorztoWords,newVertToWords,horzToWords,vert2Words,indexToHorz,indexToVert,horzToIndex,vertToIndex,used):
    """
...
def forward_look(board,indexes,indexToVert,indexToHorz,horzToIndex,vertToIndex,all_poss,horzToWords,vertToWords):
    """
    Updates data structures, then uses the updated data structures to eliminate options
    """
    newhorzToWords = horzToWords
    newvertToWords = vertToWords
    while len(indexes) > 0:
        curr = indexes.pop()
        # updates the current blocking squares
        # sets up the constraining sets for thoses newly cleared
        # printBoard(board,2,2)
        # input()
        newhorzToWords = {key:horzToWords[key].copy() for key in horzToWords}
        newvertToWords = {key:vertToWords[key].copy() for key in vertToWords}
        used = used_pieces(horzToWords,vertToWords) 
        board = update(board,curr,newhorzToWords,newvertToWords,horzToWords,vertToWords,indexToHorz,indexToVert,horzToIndex,vertToIndex,used)
        if board is None:
            return None,None,None
        # print("Old Horz to words",horzToWords)
        # print("Old vert to words",vertToWords)
        # print("new horz to words",newhorzToWords)
        # print("New vertToUsed",newvertToWords)
        # input()
        horzToWords = newhorzToWords
        vertToWords = newvertToWords
        vertStart = indexToVert[curr]
        horzStart = indexToHorz[curr]
        # print("curr",curr)
        # print("certStart",vertStart)
        # print("horz",horzStart)
        # updating the constraint sets for the index with letter currently placed
        # print(all_poss)
        for constrSet in horzToIndex,vertToIndex:
            if constrSet is horzToIndex:
                block = horzStart
            else:
                block = vertStart
            for index in constrSet[block]:
                # updates the indices in the constraining sets of curr
                if index!=curr:
                    myVertStart = indexToVert[index]
                    myHorzStart = indexToHorz[index]
                    relVertIndex = (index-myVertStart)//numCols
                    relHorzIndex = (index-myHorzStart)
                    # print(relHorzIndex,relVertIndex)
                    possHorzChars = set()
                    for word in horzToWords[myHorzStart]:
                        possHorzChars.add(word[relHorzIndex])

                    shared = set()

                    for word in vertToWords[myVertStart]:
                        if word[relVertIndex] in possHorzChars:
                            shared.add(word[relVertIndex])
                    all_poss[index] = shared
                    if len(all_poss[index]) < 1:
                        return None,None,None
                    elif len(all_poss[index]) == 1 and board[index]=="-":
                        board[index] = list(all_poss[index])[0]
                        indices.append(index)
                    # print(all_poss)
        # set the current horzToWords and vertToWords
        
                
    return board,newhorzToWords,newvertToWords
...
def poss_next(board,indToVert,indexToHorz,horzToWords,vertToWords):
    ...
    for index in range(len(board)):
        if board[index] == "-":
            ...
            if len(choices[index]) < 1:
                return None
    return choices
used = set()
start = time.perf_counter()

# indices which are blank
# filled = ["-" for i in range(4)]
startingIndices = [i for i in range(len(filled)) if filled[i].isalpha()]
all_poss = poss_next(filled,indexTovertStart,indexTohorzStart,horzWords,vertWords)
newboard,newHorz,newVert = forward_look(filled,startingIndices,indexTovertStart,indexTohorzStart,horztoIndices,vertToIndices,all_poss,horzWords,vertWords)
result = backtrackFill(filled,vertToIndices,horztoIndices,indexTovertStart,indexTohorzStart,horzWords,vertWords,all_poss)
