...
def placeBlocks(board,placed,numRows,numCols,allowed):
    ...
    while not iscontinuous(copy:=filled_board.copy(),numRows,numCols):
        start = copy.index("-")
        check_continuity(filled_board,start//numCols,start%numCols,numRows,numCols,replacement="#")
        printBoard(filled_board,numRows,numCols)
        opp = len(filled_board) - start -1
        check_continuity(filled_board,opp//numCols,opp%numCols,numRows,numCols,replacement="#")
...
def backtrack(board,allowed,numRows,numCols):
    ...
    while len(spaces) > 0:
        # choose next move
        space = random.choice(tuple(spaces))
        spaces.remove(space)
