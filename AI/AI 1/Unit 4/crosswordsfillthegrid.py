...
def backtrackFill(board,verttoIndex,horzToIndex,indexToVert,indexToHorz,horzToWords,vertToWords,used): # and other args
    ...
    for letter in poss_next(next,indToVert = indexToVert,indexToHorz=indexToHorz,horzToWords=horzToWords,vertToWords=vertToWords):
        newboard = board.copy()
        newboard[next] = letter
        newhor2words = horzToWords.copy() # shallow copy
        newvert2words = vertToWords.copy() # shallow copy
        newused = used.copy()
        update(newboard,next,newhor2words,newvert2words,indexToHorz,indexToVert,horzToIndex,verttoIndex,newused)
        # place letter and structures
        result = backtrackFill(newboard,verttoIndex,horzToIndex,indexToVert,indexToHorz,newhor2words,newvert2words,newused)
        if result is not None:
            return result
...
def update(board,index,horzToWords,vert2Words,indexToHorz,indexToVert,horzToIndex,vertToIndex,used):
    ...
    for word in horzToWords[horzstart]:
        if checkMatch(sequenceH,word):
            if word not in used or len(horzToWords[horzstart]) == 1:
                updateHorz.add(word)
    horzToWords[horzstart] = updateHorz
    # update used
    if len(updateHorz) == 1:
        val = updateHorz.pop()
        used.add(val)
    ...
    for word in vert2Words[vertStart]:
        if checkMatch(sequenceV,word):
            if word not in used or len(vert2Words[vertStart]) == 1:
                updateVert.add(word)
    vert2Words[vertStart] = updateVert
    # update used.
    if len(updateVert) == 1:
        val = updateVert.pop()
        used.add(val)
...
result = backtrackFill(filled,vertToIndices,horztoIndices,indexTovertStart,indexTohorzStart,horzWords,vertWords,used)
