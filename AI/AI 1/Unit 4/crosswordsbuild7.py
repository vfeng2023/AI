import sys; args = sys.argv[1:]# ['14x14', '108', 'v3x2', 'H2x3', 'v4x4', 'v7x4', 'H9x5', 'h8x4']# 
print(args)
import random
...
def next_space(board,numRows,numCols,spaces):
    ...
    for s in spaces:
        ...
        col = s%numCols
        count = 0
        # count the number of open spaces up down left right with the most space next to them, not just limited to three spaces
        rup = row
        while rup >= 0:
            if board[numCols*rup + col] != "#":
                count+= 1
            if board[numCols*rup+col] == "#":
                break
            rup -=1
        rright = row
        while rright < numRows:
            if board[numCols*rright + col]!="#":
                count += 1
            if board[numCols*rright+col] == "#":
                break
            rright += 1
        ...
        cleft = col
        while cleft >= 0:
            if board[numCols*row + cleft]!="#":
                count += 1
            if board[numCols*row + cleft]== "#":
                break
            cleft -= 1
        cright = col
        while cright < numCols:
            if board[numRows*row+cright]!="#":
                count += 1
            if board[numCols*row+cleft]:
                break
            cright += 1
