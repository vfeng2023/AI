import sys; args = ["5x5","0","twentyk.txt"]# args = sys.argv[1:] 
import random
import time
...
def backtrackFill(board,verttoIndex,horzToIndex,indexToVert,indexToHorz,horzToWords,vertToWords): # and other args
    ...
    # print("Used",used)
    # input()
    next = get_next(board,verttoIndex,horzToIndex)
    possible_letters = poss_next(next,indToVert = indexToVert,indexToHorz=indexToHorz,horzToWords=horzToWords,vertToWords=vertToWords)
    ...
    for letter in possible_letters:
        ...
        update(newboard,next,newhor2words,newvert2words,horzToWords,vertToWords,indexToHorz,indexToVert,horzToIndex,verttoIndex,newused)
        # place letter and structures
        result = backtrackFill(newboard,verttoIndex,horzToIndex,indexToVert,indexToHorz,newhor2words,newvert2words)
        if result is not None:
            return result
...
def update(board,index,newhorztoWords,newVertToWords,horzToWords,vert2Words,indexToHorz,indexToVert,horzToIndex,vertToIndex,used):
    ...
    for word in horzToWords[horzstart]:
        if len(horzToWords[horzstart]) > 1:
            if not checkMatch(sequenceH,word) or word in used:
                # if newhorztoWords[horzstart] is horzToWords[horzstart]:
                #     print(newhorztoWords[horzstart])
                #     print(horzToWords[horzstart])
                #     print("Same")
                # else:
    ...
    # update used
    if len(newhorztoWords[horzstart]) == 1:
        val = newhorztoWords[horzstart].pop()
        used.add(val)
        newhorztoWords[horzstart].add(val)
    if len(newhorztoWords[horzstart]) < 1:
        return False
    # for H and V
    # generate new sequence
    # new set
    # add only the words which match the new sequence
    # update dict
    # update the vertical dictionarry
    ...
    # update used.
    if len(newVertToWords[vertStart]) == 1:
        val = newVertToWords[vertStart].pop()
        used.add(val)
        newVertToWords[vertStart].add(val)
    if len(newVertToWords[vertStart]) < 1:
        return False
    return True
    # eliminate used words as a possibility from all dictionaries
    # for key in vert2Words:
    #     if key!=vertStart:
    #         for word in used:
    #             if word in vert2Words[key]:
    #                 vert2Words[key].remove(word)
    # for key in horzToWords:
    #     if key!=horzstart:
    #         for word in used:
    #             if word in horzToWords[key]:
    #                 horzToWords[key].remove(word)
...
def get_next(board,verttoIndex,horzToIndex):
    """
    Returns the next open index in the space the with fewest open spaces
    """
    bestSpace = -1,None
    minOpen = float('inf')
    for v in verttoIndex:
        # count open
        sequence = getSequence(board,v,verttoIndex)
        openSpace = sequence.count("-")
        if openSpace == 0:
            continue
        # letters = len(sequence) - openSpace

        if openSpace < minOpen:
            bestSpace = v,"V"
            minOpen = openSpace
    for h in horzToIndex:
        # count open
        sequence = getSequence(board,h,horzToIndex)
        openSpace = sequence.count("-")
        if openSpace == 0:
            continue
        letters = len(sequence) - openSpace
    ...
    space,direction = bestSpace
    if direction == "H":
        indices = horzToIndex[space]
    elif direction == "V":
        indices = verttoIndex[space]
...
# def get_next(board,verttoIndex,horzToIndex):
#     """
#     Returns the next open index in the space the with most open spaces
#     """
#     bestSpace = -1,None
#     minOpen = -float('inf')
#     for v in verttoIndex:
...
# def get_next(board,vertToIndex,horzToIndex):
#     """
#     Returns the most restrictive space on the board
#      X definition of restrictive: contained in the constraining set(horizontal and vertical words) that has the most constraining effect: i.e the sum of the total spaces in the constraining set it belongs to
     
#     """
#     mostconstr = float('inf')
#     mostConstrindex = -1
...
#             # #
#             # openSpaces = len(vertToIndex[vertstart])+len(horzToIndex[horzstart])
#             # if openSpaces > mostconstr:
#             #     mostconstr = openSpaces
#             #     mostConstrindex = index
#             # return least open spaces in constraining set
#             openSpaces = 0
#             for i in horzToIndex[horzstart]:
#                 if board[i] == "-":
#                     openSpaces += 1
#             for j in vertToIndex[vertstart]:
#                 if board[j] == "-":
#                     openSpaces += 1
#             if openSpaces < mostconstr:
#                 mostConstrindex = index
#                 mostconstr = openSpaces
#     return mostConstrindex

            
            
def poss_next(index,indToVert,indexToHorz,horzToWords,vertToWords):
