...
def a_star(start,size):
    ...
    if start[0].isdigit() or start[1].isdigit():
        mappings = {ch:int(ch) for ch in puzzle if ch!="."}
    closed = set()
    start_depth = 0
        ...
        if state not in closed:
            ...
            for child in children:
                if child not in closed:
                    heappush(fringe,(depth + (taxicab(child,size,mappings,state,f),child,depth+1))
    return None
...
def taxicab(puzzle,size,indices,prev_state,prev_dist):
    if prev_state == "":
        distance = 0
        for i in range(len(puzzle)):
            if puzzle[i]!=".":
        ...
        dot = prev_state.find(".")
        loc = puzzle[dot]
        return prev_dist + abs(loc//size-dot//size) + abs(loc%size-dot%size)
