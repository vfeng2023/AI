...
Only uses # of holes an roughness
"""
import sys
import random
import time
...
def place_piece(board,piece,orientation,location,column_heights):
    ...
    for col in range(len(column_heights)):
        ...
        column_heights[col] = rindex(new_board,col,column_heights[col])
...
def play_game(strategy,nicely=False):
    ...
    while board != "GAME OVER":
        ...
        if nicely:
            ...
            print("Points: ",points)
            # numHoles,roughness = highest_heap(board,column_heights)
            # print("numholes, roughness: ",numHoles,roughness)
            input()
            # points += new_points
        # print_board(board)
...
def heurestic(board,strategy,rowsCleared,columns):
    ...
    value = 0
    fc,fe,gc,ge = strategy
    numHoles,roughness = highest_heap(board,columns)
...
# board = " #########"*4 + " ##" + " "*157
# print_board(board)
# column_heigts = [0,5,5,4,4,4,4,4,4,4]
# board,_=place_piece(board,"I",1,0,column_heigts)
# print_board(board)
# print("New Colmns: ",column_heigts)
# strat = (0.76,3,-0.2,2)
# play_game(strat,nicely=True)
