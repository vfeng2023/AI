...
def naive8Color(filename):
    ...
    img.save(filename+'.naive8color.png')
        ...
        for y in range(height):
            ...
            for i in range(3):
                if color[i] < 255//3:
                    ...
                    newColor[i] = 255
            pix[x,y] = tuple(newColor)
    img.save(filename+'.naive27color.png')
    ...
    for m in meanGroups:
        ...
        for pix in meanGroups[m]:
            for x,y in pixelsCount[pix]:
               imgPix[x,y] = pizm
    img.show()
    img.save(filename+'.kmeansout{0}.png'.format(K))
...
#     (255,255,255):[(0,0),(1,1)]
# }
# pixelList = [(0,0,0),(255,255,255)]
# print(calc_avg(pixelCount,pixelList))