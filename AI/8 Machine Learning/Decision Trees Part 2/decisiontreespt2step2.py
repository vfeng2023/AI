...
def classify_helper(dataPt,tree):
    ...
    for k in tree:
        ...
        if myVal not in tree[k]:
            return classify_helper(dataPt,tree[k])
        else:
...
with open('house-votes-84.csv') as f:
    ...
    data_set = []
    missing_vals = []
    feature_names = lines[0].split(",")[1:]
    for line in lines[1:]:
        if "?" not in line:
            dataPt = line.split(",")[1:]
            data_set.append(dataPt)
        else:
            missing_vals.append(line.split(",")[1:])
    ...
    print(len(data_set))
    # input()
    train_pool = data_set[0:len(data_set)-50]
    # print(train_pool)
    testing_set = data_set[len(data_set)-50:len(data_set)]
    # print(testing_set)
